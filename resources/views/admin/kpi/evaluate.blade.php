<x-layout-admin :title="'Evaluasi KPI Karyawan'">
    @push('styles')
    <style>
        /* == Background == */
    ...
    <div class="flex flex-col flex-1 min-h-screen mesh-bg relative overflow-hidden p-4 sm:p-6 md:p-8">
        ...
        <div class="max-w-7xl w-full mx-auto relative z-10">
            ...
            <a href="{{ route('admin.kpi.index') }}" class="btn-back-modern">
            ...
            {{-- Main Form --}}
            <form action="{{ route('admin.kpi.storeEvaluate', $targetUser->id) }}" method="POST" id="kpi-form-umum" class="space-y-6">
                @csrf
                                ...
                                <table class="w-full text-left border-collapse min-w-[900px]">
                                    ...
                                    <tbody class="divide-y divide-gray-200">
                                        ...
                                        <tr class="bg-blue-50/50 font-bold border-t-2 border-blue-100">
                                            <td colspan="7" class="p-3 text-right text-blue-900">Total Kinerja (100% dari 60%)</td>
                ...
                <div class="flex flex-col sm:flex-row justify-end gap-4 pt-6 border-t border-gray-200">
                    <a href="{{ route('admin.kpi.index') }}" class="px-6 py-3 bg-white text-slate-700 border border-slate-300 rounded-xl hover:bg-slate-50 font-bold text-center transition-colors shadow-sm flex items-center justify-center gap-2">
            ...
            <div class="flex flex-col sm:flex-row justify-end gap-4 mt-4">
                @if($evaluation && $evaluation->status == 'disetujui_kepala_divisi' && (Auth::user()->role === 'direktur' || strtolower(Auth::user()->divisi) === 'top management' || Auth::user()->role === 'admin'))
                    <form action="{{ route('admin.kpi.approve', $evaluation->id) }}" method="POST">
                        @csrf
                        <button type="submit" class="px-8 py-3 bg-gradient-to-r from-emerald-500 to-teal-400 text-white rounded-xl hover:from-emerald-600 hover:to-teal-500 font-bold shadow-lg shadow-emerald-500/30 transform transition-all hover:-translate-y-0.5 text-center flex items-center justify-center gap-2">
                            <i class="fas fa-check-double"></i> Approve Evaluasi
                        </button>
                    </form>
                @endif
            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const inputs = document.querySelectorAll('.kpi-input-umum, .kpi-eval-input');
            ...
            function calculateTotals() {
                ...
                inputs.forEach(input => {
                    ...
                    const group = input.dataset.group || (input.classList.contains('kpi-eval-input') ? 'kinerja' : '');
                    const row = input.closest('tr');
                    const finalInput = row.querySelector('.final-score-umum, .final-score-input');
                ...
                // Wait, based on KpiController it's already calculated as finalScore, let's keep it consistent
                const kpiKinerja = totalKinerjaScore * 0.6; // Assuming original logic
