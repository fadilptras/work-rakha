    ...
    <div>
        <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center">
            <div>
                ...
                <p class="text-sm text-zinc-400 mt-1">Mengatur urutan persetujuan (Approver 1 hingga 4) untuk pengajuan dana.</p>
            </div>
        </div>
                                ...
                                <tr class="hover:bg-zinc-700/50 approver-row">
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <div class="relative w-full min-w-[150px] max-w-[180px]">
                                            ...
                                            <select name="approver_3[{{ $employee->id }}]" 
                                                    class="w-full appearance-none p-2 pr-8 bg-zinc-700 border border-zinc-600 rounded-lg text-xs text-white focus:ring-sky-500 focus:border-sky-500 approver-select">
                                                <option value="">-- Tidak Ada --</option>
                                                @foreach ($approvers as $approver)
                                                    <option value="{{ $approver->id }}" @selected($employee->approver_dana_3_id == $approver->id)>
                                                        {{ $approver->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <div class="relative w-full min-w-[150px] max-w-[180px]">
                                            ...
                                            <select name="approver_4[{{ $employee->id }}]" 
                                                    class="w-full appearance-none p-2 pr-8 bg-zinc-700 border border-zinc-600 rounded-lg text-xs text-white focus:ring-sky-500 focus:border-sky-500 approver-select">
                                                <option value="">-- Tidak Ada --</option>
                                                @foreach ($admins as $adminData)
                                                    <option value="{{ $adminData->id }}" @selected($employee->approver_dana_4_id == $adminData->id)>
                                                        {{ $adminData->name }}
                                                    </option>
                                                @endforeach
                                            </select>
