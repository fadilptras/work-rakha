...
<x-layout-users>
    ...
    <div class="flex flex-col flex-1 min-h-screen mesh-bg relative overflow-hidden">
        ...
        <div class="relative z-10 w-full max-w-6xl mx-auto p-4 sm:p-6 lg:p-8 flex-1 flex flex-col">
            ...
            <a href="{{ route('pengajuan_dana.index') }}" class="btn-back-modern">
            ...
            <div class="relative z-10 w-full bg-gradient-to-r from-blue-700 to-indigo-600 rounded-2xl md:rounded-3xl p-5 md:p-6 shadow-xl mb-4 md:mb-6 overflow-hidden border border-white/20">
                ...
                <div class="relative z-10 flex items-center gap-3 md:gap-4">
                    <div class="h-10 w-10 md:h-12 md:w-12 rounded-xl md:rounded-2xl bg-white/20 flex items-center justify-center backdrop-blur-md border border-white/20 flex-shrink-0">
                        <i class="fas fa-desktop text-lg md:text-xl text-white"></i>
                    </div>
                    <div class="flex-1">
                        ...
                        <p class="text-blue-100 text-xs md:text-sm mt-0.5 font-medium leading-relaxed w-full">
                            Pantau status, detail, dan alur persetujuan seluruh pengajuan dana operasional/kantor.
                        </p>
                        @endif
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-4 border-b border-slate-200 pb-4 mb-5">
                    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h3 class="text-lg font-black text-slate-800">Daftar Pengajuan Dana</h3>
                                @if(!$isMobile)
                    ...
                    <div class="flex justify-end mt-1">
                        <form action="{{ route('pengajuan_dana.monitoring_all') }}" method="GET" class="flex flex-col md:flex-row items-center gap-2.5 w-full md:w-auto m-0">
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama, judul..." class="modern-select" style="width: 100%; md:max-width: 220px;">
                            <div class="flex items-center gap-2 w-full md:w-auto">
                                <select name="status" class="modern-select w-full md:w-auto" onchange="this.form.submit()">
                                    ...
                                    <option value="diajukan" {{ request('status') == 'diajukan' ? 'selected' : '' }}>Menunggu Persetujuan</option>
                                    <option value="diproses" {{ request('status') == 'diproses' ? 'selected' : '' }}>Diproses (Belum Selesai)</option>
                                    <option value="disetujui" {{ request('status') == 'disetujui' ? 'selected' : '' }}>Selesai Disetujui</option>
                ...
                <div class="overflow-x-auto hidden md:block rounded-2xl border border-slate-200/80" style="border-radius: 16px; overflow: hidden;">
                    <table class="min-w-full text-xs">
                        <thead class="bg-slate-50/80 text-slate-600 uppercase font-black border-b border-slate-200">
                            <tr>
                                <th class="px-6 py-4 text-left w-[10%]">Tanggal</th>
                                <th class="px-6 py-4 text-left w-[20%]">Pemohon</th>
                                <th class="px-6 py-4 text-left w-[30%]">Judul Pengajuan</th>
                                <th class="px-6 py-4 text-left w-[15%]">Total Dana</th>
                                <th class="px-6 py-4 text-left w-[15%]">Status</th>
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                            @forelse ($pengajuanDanas as $pengajuan)
                                @php
                                    $statusClass = match($pengajuan->status) {
                                        ...
                                        'proses_pembayaran' => 'bg-indigo-100 text-indigo-700 border-indigo-200',
                                        default => 'bg-slate-100 text-slate-700 border-slate-200',
                                    };
                                    $statusLabel = match($pengajuan->status) {
                                        'diajukan' => 'Menunggu',
                                        'diproses' => 'Diproses',
                                        'proses_pembayaran' => 'Proses Bayar',
                                        'disetujui' => 'Disetujui',
                                        'ditolak' => 'Ditolak',
                                ...
                                <tr class="hover:bg-slate-50/50 transition">
                                    <td class="px-6 py-4.5 text-slate-500 font-extrabold">{{ $pengajuan->created_at->locale('id')->isoFormat('D MMM YYYY') }}</td>
                                    <td class="px-6 py-4.5 text-slate-800 font-bold">
                                        {{ $pengajuan->user->name ?? 'Unknown' }}
                                        <div class="text-[10px] text-slate-400 font-normal mt-0.5">{{ $pengajuan->divisi ?? '-' }}</div>
                                    </td>
                                    <td class="px-6 py-4.5 text-slate-800 font-bold leading-normal">
                                        {{ $pengajuan->judul_pengajuan }}
                                    </td>
                                    <td class="px-6 py-4.5 font-black text-slate-700 text-blue-600">Rp {{ number_format($pengajuan->total_dana, 0, ',', '.') }}</td>
                                    ...
                                    <td class="px-6 py-4.5 text-center">
                                        <a href="{{ route('pengajuan_dana.show', ['pengajuanDana' => $pengajuan->id, 'from' => 'monitoring']) }}" class="inline-flex items-center gap-1 text-xs text-blue-600 font-black hover:underline bg-blue-50/70 border border-blue-100 px-3.5 py-2 rounded-xl transition hover:bg-blue-100 shadow-sm">
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="text-center text-slate-400" style="padding: 56px 24px;">
                                        <div class="flex flex-col items-center justify-center">
                                            <i class="fas fa-folder-open text-4xl text-slate-200" style="margin-bottom: 12px;"></i>
                                            <p class="text-xs font-bold text-slate-500 uppercase tracking-wider">Belum ada data pengajuan dana yang sesuai.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
                ...
                <div class="block md:hidden space-y-4">
                    @forelse ($pengajuanDanas as $pengajuan)
                        ...
                        <div class="history-card-mobile rounded-2xl p-4 border shadow-sm space-y-3">
                            ...
                            <div class="flex justify-between">
                                <div class="text-[11px] text-slate-500 font-bold"><i class="fas fa-user text-[10px] mr-1"></i>{{ $pengajuan->user->name ?? 'Unknown' }}</div>
                                <div class="text-[11px] text-slate-400 font-bold">{{ $pengajuan->created_at->locale('id')->isoFormat('D MMMM YYYY') }}</div>
                            </div>
                            <div class="flex justify-between items-center pt-3 border-t border-slate-100">
                                <div class="text-[11px] text-blue-600 font-black">Rp {{ number_format($pengajuan->total_dana, 0, ',', '.') }}</div>
                                <a href="{{ route('pengajuan_dana.show', ['pengajuanDana' => $pengajuan->id, 'from' => 'monitoring']) }}" class="text-xs text-blue-600 font-bold hover:underline">
                                    Detail <i class="fas fa-chevron-right text-[8px] ml-0.5"></i>
                                </a>
                            </div>
                        </div>
                    @empty
                        <div class="text-center text-slate-400 py-10 bg-white rounded-2xl border border-slate-200">
                            ...
                            <p class="text-xs font-semibold text-slate-500">Belum ada data pengajuan dana yang sesuai.</p>
                        </div>
                    @endforelse
                </div>

                {{-- Pagination --}}
                @if ($pengajuanDanas->hasPages())
