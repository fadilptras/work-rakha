<x-layout-users>
    ...
    <style>
        ...
        .weekly-strip {
            background: linear-gradient(135deg, #f7fdf9 0%, #e6f7ec 100%);
        ...
        .weekly-strip .week-nav-btn:hover { background: #d1fae5; color: #047857; }
        ...
        .weekly-strip .day-num:hover { background: #d1fae5; }
        ...
        /* == Welcome Card Mobile == */
        .mobile-welcome-card {
            background: linear-gradient(135deg, #001BB7 0%, #1d4ed8 50%, #312e81 100%);
            border-radius: 1.5rem;
            padding: 24px 20px;
            color: #fff;
            display: flex;
            ...
            position: relative;
            overflow: hidden;
            box-shadow: 0 10px 25px -5px rgba(0, 27, 183, 0.4);
        ...
        <div class="mobile-welcome-card">
            ...
            <div class="avatar-circle">
                @if(Auth::user()->profile_picture)
                    <img class="w-full h-full object-cover rounded-full" src="{{ asset('storage/' . Auth::user()->profile_picture) }}" alt="Profil">
                @else
                    <i class="fas fa-user-tie text-xl text-white/80"></i>
                @endif
            </div>
        </div>
    </div>
    ...
    {{-- Divisi Info --}}
    <div class="px-5 pb-3">
        <div class="flex items-center gap-2 bg-white/80 backdrop-blur-md rounded-2xl px-4 py-3 shadow-xl shadow-blue-900/5 border border-white">
    ...
    {{-- Absensi Section --}}
    <div class="px-5 pb-5">
        <div class="bg-white/80 backdrop-blur-md rounded-3xl p-5 shadow-xl shadow-blue-900/5 border border-white">
            <h3 class="font-bold text-gray-800 text-base mb-4 flex items-center gap-2">
                <i class="fas fa-clock text-indigo-500"></i> Pintasan Absensi
            </h3>
            <div class="grid grid-cols-2 gap-3">
                <a href="{{ route('absen') }}" class="btn-absen-soft py-4 px-2 rounded-2xl text-center flex items-center justify-center transition-all duration-300">
                    <span class="font-extrabold text-sm tracking-wide text-current">Absen</span>
                </a>
                <a href="{{ route('aktivitas.index') }}" class="btn-aktivitas-soft py-4 px-2 rounded-2xl text-center flex items-center justify-center transition-all duration-300">
                    <span class="font-extrabold text-sm tracking-wide text-current">Aktivitas</span>
                </a>
                <a href="{{ route('cuti.create') }}" class="btn-cuti-soft py-4 px-2 rounded-2xl text-center flex items-center justify-center transition-all duration-300">
                    <span class="font-extrabold text-sm tracking-wide text-current">Cuti</span>
                </a>
                <a href="{{ route('rekap_absen.index') }}" class="btn-rekap-soft py-4 px-2 rounded-2xl text-center flex items-center justify-center transition-all duration-300">
    ...
    {{-- Agenda Hari Ini --}}
    <div class="px-5 pb-5">
        <div class="card-pastel-green-age rounded-3xl p-5 text-slate-800 flex flex-col">
            <div class="flex items-center justify-between mb-4 flex-shrink-0">
                <h3 class="font-extrabold text-emerald-950 text-lg flex items-center gap-2">
                    <i class="fas fa-calendar-check text-emerald-600"></i> Agenda Hari Ini
                </h3>
                <button id="mobile-add-agenda-btn" class="bg-gradient-to-r from-emerald-600 to-green-600 hover:brightness-110 text-white font-bold w-9 h-9 rounded-full transition-all duration-200 flex items-center justify-center shadow-lg shadow-green-600/20 hover:scale-105">
                    <i class="fas fa-plus text-xs"></i>
                </button>
            </div>
            <div id="mobile-agenda-today-container">
                <div class="mobile-agenda-empty flex flex-col items-center justify-center py-6 bg-white/50 rounded-2xl border border-white/60">
                    <div class="w-12 h-12 bg-emerald-100 rounded-full flex items-center justify-center mb-3 shadow-inner">
                        <i class="fas fa-calendar-alt text-xl text-emerald-500"></i>
                    </div>
                    <p class="font-bold text-emerald-900 text-sm">Tidak ada agenda</p>
                    <p class="text-xs text-emerald-600 mt-1">Tap + untuk menambah jadwal.</p>
                </div>
            </div>
        </div>
    </div>
    ...
    @push('modals')
    {{-- KONTEN MODAL --}}
    <div id="agenda-modal" class="fixed inset-0 bg-black bg-opacity-60 z-[9999] hidden flex items-center justify-center p-4">
        <div class="modal-card max-w-3xl w-full max-h-[75vh]" id="agenda-modal-content">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="space-y-3">
                            ...
                            <div>
                                ...
                                <textarea id="description" name="description" rows="2" class="modal-input" placeholder="Jelaskan detail agenda di sini..."></textarea>
                            </div>
                        ...
                        <div class="space-y-3">
                            <div>
                                ...
                                <div id="guest-list-container" class="guest-list-card custom-scrollbar" style="height: 85px;">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input type="text" id="location" name="location" class="modal-input text-xs" placeholder="Ruang Meeting Lt. 2">
                                </div>
                                <div>
                                    <label for="color" class="modal-label">Warna Label</label>
                                    <div class="color-picker-wrapper mt-0.5">
    ...
    <div id="agenda-detail-modal" class="fixed inset-0 bg-black bg-opacity-60 z-[9999] hidden flex items-center justify-center p-4">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            try {
                ...
                const modalTitle = agendaModal ? agendaModal.querySelector('h4') : null;
                const saveButton = document.getElementById('save-agenda-btn');

                // == Attach Mobile Add Agenda Button EARLY ==
                if (mobileAddAgendaBtn) {
                    mobileAddAgendaBtn.addEventListener('click', function(e) {
                        e.preventDefault();
                        openModalForCreate();
                    });
                }
                ...
                function updateAgendaList(selectedDate) {
                    ...
                    if (agendaListTitle) agendaListTitle.textContent = `Agenda (${startFormatted} - ${endFormatted})`;
                ...
                function closeDetailModal() { if (detailModal) detailModal.classList.add('hidden'); }
                ...
                function openModalForCreate() {
                    try {
                        ...
                        const titleText = document.getElementById('modal-agenda-title-text');
                        if (titleText) titleText.textContent = 'Buat Agenda Baru';
                        if (saveButton) saveButton.textContent = 'Simpan Agenda';
                        
                        const colorInput = document.getElementById('color');
                        if (colorInput) colorInput.value = '#3B82F6';
                        
                        if (agendaDate && typeof agendaDate.setDate === 'function') agendaDate.setDate(new Date());
                        if (startHour && typeof startHour.clear === 'function') startHour.clear();
                        if (endHour && typeof endHour.clear === 'function') endHour.clear();
                ...
                function closeModal() { if (agendaModal) agendaModal.classList.add('hidden'); }

                if (addAgendaBtn) addAgendaBtn.addEventListener('click', openModalForCreate);
                if (closeModalBtn) closeModalBtn.addEventListener('click', closeModal);
                if (cancelBtn) cancelBtn.addEventListener('click', closeModal);
                if (agendaModal) agendaModal.addEventListener('click', (e) => { if (e.target === agendaModal) closeModal(); });
                ...
                console.error("Initialization error in dashboard_mobile JS:", initError);
            }
        });
    </script>
